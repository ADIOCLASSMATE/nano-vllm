...
class Config:
    ...
    def __post_init__(self):
        ...
        if "llada" in cfg.model_type.lower():
        ...
        elif "dream" in cfg.model_type.lower():
            # Dream model config (Qwen2-style)
            ...
            self.torch_dtype            = getattr(cfg, "torch_dtype", torch.bfloat16)
            if isinstance(self.torch_dtype, str):
                self.torch_dtype = getattr(torch, self.torch_dtype.replace("torch.", ""))
        else:
            # standard HF configs
            self.hidden_size            = get_cfg_alias(cfg, "hidden_size")
            self.num_attention_heads    = get_cfg_alias(cfg, "num_attention_heads")
